...
- `CAMPAIGN_IMMEDIATE_PAUSE_NO_SESSIONS`, `CAMPAIGN_POOL_AUTO_ADJUST`,
  ...
  `CAMPAIGN_WAIT_FOR_SESSIONS_MS`, `CAMPAIGN_PREVERIFY_SESSION_COUNT`,
  `CAMPAIGN_PREVERIFY_FULL_POOL`, `CAMPAIGN_PREVERIFY_CONCURRENCY`,
  `CAMPAIGN_RATE_LIMIT_RESUME_MODE` (`server/campaignEngine.ts`).
...
- `WHATSAPP_RAMP_UP_ENABLED` (`server/healthPolicy.ts`).
